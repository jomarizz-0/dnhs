...
function renderTable() {
  ...
  tbody.innerHTML = page.map(s => {
    ...
    const bday = s.birth_date ? new Date(s.birth_date).toLocaleDateString('en-PH', {year:'numeric',month:'short',day:'numeric'}) : '—';
    ...
    return `
    <tr>
      <td class="td-id">#${String(s.student_id).padStart(4,'0')}</td>
      ...
      <td>
        <div class="td-actions">
          <button class="btn btn-edit" onclick="openEditModal(${s.student_id})">Edit</button>
          <button class="btn btn-danger btn-sm" onclick="openConfirm(${s.student_id}, '${escHtml(fullName).replace(/'/g, "\\'")}')">Delete</button>
        </div>
      </td>
    </tr>`;
  }).join('');
...
function openConfirm(id, name) {
  ...
  document.getElementById('confirmDeleteBtn').onclick = async () => {
    const res  = await fetch(`${API}?action=delete&id=${deleteId}`, { method: 'DELETE' });
    const data = await res.json();
    closeConfirm();
    if (data.success) {
      fetchStudents(document.getElementById('searchInput').value);
...
// ── Helpers ──────────────────────────────────────────────────────────────────
function clearForm() {
  ['fFirstName','fLastName','fMiddleName','fBirthdate','fBarangay','fMunicipality'].forEach(id => {
